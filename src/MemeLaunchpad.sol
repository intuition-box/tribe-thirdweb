...
contract MemeLaunchpad {
    ...
    event TokensSold(address indexed token,address indexed seller,uint256 tokens,uint256 payment);
    ...
    /// @notice Buy tokens from bonding curve. Send native currency with tx.
    function buyTokens(address t,uint256 minOut) external payable nonReentrant onlyValidToken(t) returns(uint256 bought) {
        TokenInfo storage ti=tokenInfo[t]; uint256 payment=msg.value;
        if(ti.completed)revert InvalidInput(); if(payment==0)revert MustSendPayment(); if(!tokenUnlocked[t]&&msg.sender!=ti.creator)revert TokenLocked();
        uint256 excess=_getExcessLiquidity(t); uint256 price=_calculatePrice(ti.currentSupply,excess);
        ...
        _processBuyCompletionIfReached(t,ti,bondMax,excess);
    ...
    // --- Sell (bonding curve) ---
    /// @notice Sell tokens back to bonding curve. Approve this contract first.
    function sellTokens(address t,uint256 amount,uint256 minOut,address payoutTo) external nonReentrant onlyValidToken(t) returns(uint256 net) {
        TokenInfo storage ti=tokenInfo[t]; if(ti.completed)revert TokenLaunchCompleted(); if(amount==0)revert MustSellTokens();
        (uint256 calcPay,uint256 toDist)=_calculateSellPayment(t,msg.sender,amount,minOut);
        net=_processSellTokenOps(t,msg.sender,amount,calcPay,toDist);
        _processSellPayout(t,msg.sender,payoutTo,net,toDist,amount); return net;
    }
    ...
    function _calculateSellPayment(address t,address seller,uint256 amount,uint256 minOut) private view returns(uint256 calcPay,uint256 toDist) {
        MemeToken tc=MemeToken(payable(t)); TokenInfo storage ti=tokenInfo[t];
        if(tc.balanceOf(seller)<amount)revert InsufficientTokenBalance(); if(ti.currentSupply<amount||ti.currentSupply==0)revert InsufficientCirculatingSupply();
        uint256 purchased=userTokenPurchases[t][seller]; uint256 contrib=userContributions[t][seller];
        if(purchased==0||contrib==0)revert NoTokensPurchased(); if(amount>purchased)revert InsufficientTokenBalance();
        calcPay=(contrib*amount)/purchased; if(calcPay==0)revert MustSellTokens();
        uint256 spread=hasTokenSellSpread[t]?tokenSellSpreadPercent[t]:sellSpreadPercent;
        toDist=(calcPay*(100-spread))/100; if(tc.getNativeBalance()<toDist)revert InsufficientBondingCurveLiquidity();
        uint256 fee=(toDist*feePercent)/100; uint256 net; unchecked{net=toDist-fee;}
    ...
    function _processSellTokenOps(address t,address seller,uint256 amount,uint256 calcPay,uint256 toDist) private returns(uint256 net) {
        TokenInfo storage ti=tokenInfo[t]; MemeToken tc=MemeToken(payable(t));
        unchecked{ti.currentSupply-=amount;}
        tc.burnFrom(seller,amount); if(!tc.transferNative(payable(address(this)),toDist))revert TransferFailed();
        unchecked{curveLiquidity[t]-=toDist;}
        uint256 purchased=userTokenPurchases[t][seller];
        if(amount>=purchased){ delete userTokenPurchases[t][seller]; delete userContributions[t][seller]; }
        else{ unchecked{ userTokenPurchases[t][seller]-=amount; userContributions[t][seller]-=calcPay; } }
        uint256 fee=(toDist*feePercent)/100; unchecked{net=toDist-fee;}
    }
    function _processSellPayout(address t,address seller,address payoutTo,uint256 net,uint256 toDist,uint256 amount) private {
        address to=payoutTo==address(0)?seller:payoutTo;
        (bool ok,)=payable(to).call{value:net}(""); if(!ok)revert TransferFailed();
        (ok,)=payable(treasuryAddress).call{value:toDist-net}(""); if(!ok)revert TransferFailed();
        unchecked{ userVolumes[seller].totalSellVolume+=net; tokenTotalValueTraded[t]+=net; }
        emit TokensSold(t,seller,amount,net);
    }
    ...
    // --- Pricing (quadratic curve; floor raised by excess sell-spread liquidity) ---
    function _getExcessLiquidity(address t) internal view returns(uint256){ uint256 b=MemeToken(payable(t)).getNativeBalance(); uint256 tr=curveLiquidity[t]; return b>tr?b-tr:0; }
    function _calculatePrice(uint256 supply,uint256 excess) internal pure returns(uint256) {
        ...
        if(supply==0)return effInit; uint256 ratio=(supply*1e18)/PRICE_STEP_SIZE; return(effInit*(1e18+(ratio*ratio)/1e18))/1e18;
    }
    function getCurrentPrice(address t) public view onlyValidToken(t) returns(uint256){ return _calculatePrice(tokenInfo[t].currentSupply,_getExcessLiquidity(t)); }
    ...
    function auditTrustAccounting(address t) external view onlyValidToken(t) returns(bool ok,uint256 curve,uint256 bal,uint256 diff) {
        curve=curveLiquidity[t]; bal=MemeToken(payable(t)).getNativeBalance();         ok=bal>=curve; diff=bal>=curve?bal-curve:0;
    }

    // --- Comments & Transfer Fees ---
    function addComment(address t,string calldata text) external payable nonReentrant onlyValidToken(t) {
        if(bytes(text).length==0||msg.value!=COMMENT_FEE)revert InvalidInput();
        tokenComments[t].push(Comment(msg.sender,text,block.timestamp));
        (bool ok,)=payable(treasuryAddress).call{value:COMMENT_FEE}(""); if(!ok)revert TransferFailed();
        emit TokenCommented(t,msg.sender,text,block.timestamp);
    }

    function collectAndSplitTransferFees(address t) external onlyValidToken(t) {
        ...
        uint256 creator=acc/2; uint256 launchpad=acc-creator;
        if(creator>0&&!tc.transfer(tokenInfo[t].creator,creator))revert TransferFailed();
        emit TransferFeeCollected(t,address(0),acc,creator,launchpad);
    }
    ...
    /// @notice Refund all users' native contributions (owner only). Does not burn tokens.
    function emergencyWithdrawTokens(address t) external onlyOwner onlyValidToken(t) {
    ...
    function completeTokenLaunch(address t) external onlyOwner onlyValidToken(t) {
